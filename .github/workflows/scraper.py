                ...
                if len(cols) >= 3:
                    ...
                    if code_match and weight_text:
                        try:
                            ...
                            pass
        if holdings:
            logging.info(f"✅ {etf_code} 抓取成功，共 {len(holdings)} 筆")
            return holdings
    except Exception as e:
        logging.warning(f"⚠️ {etf_code} 失敗：{e}")
    return {}
...
def main():
    ...
    logging.info(f"✅ 完成！MoneyDJ 成功 {success}/{len(ETF_META)} 支，已儲存 data.json")
